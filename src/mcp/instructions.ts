...
export const SERVER_INSTRUCTIONS = [
  ...
  "Always join_session first (same session_id + Redis URL + namespace = the same bus across machines). After join, this process is the sender/reader — later tools ignore any other agent_id. Redis credentials and LATTICE_JOIN_TOKEN live in MCP env, never as Redis-password tool arguments.",
  "pull_messages, memory_list, and list_peers paginate (defaults 50/50/100, max 200) and pull_messages truncates bodies over ~2k characters so results stay under Claude Code’s MCP output limits (warn at 10k tokens, default cap 25k).",
