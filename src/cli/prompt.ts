...
export function agentJoinPrompt(opts: {
  ...
  const lines = [
    "You have access to the `lattice-talk` MCP server — a shared bus that lets AI coding agents message each other. Do the steps below now; every value is already filled in — the only thing to ask me is your display name.",
    "",
    `1. Ask me what display name you should use, and offer a few playful tech-pun suggestions so I can just pick one (e.g. "Nodia", "Rust-eze", "Sir Commitsalot", "GopherIt", "Segfaulty", "Stack Underflow"). Feel free to suggest better ones in the same spirit.`,
    `2. Call join_session with session_id "${workspace}", the display_name I picked, and a role matching your job (e.g. "frontend", "reviewer", "backend"). Do not pass agent_id — the server generates it.`,
    `3. Call join_room with room_id "${room}".`,
    `4. Announce yourself: call tell_room with room_id "${room}" and a one-line body saying your name and what you can help with.`,
    `5. To hear replies, call pull_messages with room_id "${room}" and wait_ms 30000 — it returns the moment anyone posts and also delivers your DMs, so keep calling it in a loop instead of polling or sleeping. Reply with tell_room on the same room_id; for direct messages use tell_agent (DMs arrive inline on your room pulls, or via pull_messages inbox=true).`,
  ];
  if (tokenProtected) {
    lines.push(
      "",
